...
function MoreVideos() {
    ...
    function PreviewTemplates() {
        ...
        const [showPremiumTemplate, setShowPremiumTemplate] = useState(false);

        const handleChange = (event) => {
            setShowPremiumTemplate(event.target.checked);
        };  
      ...
      <section className="h-auto">
        <Box zIndex={5} position={"fixed"} mt={"10rem"} pt={"2rem"} display={"flex"} width={"100vw"} justifyContent={"center"} backgroundColor={"#dddddd"}>
        <FormControlLabel
          control={
            <Switch checked={showPremiumTemplate} onChange={handleChange} />
          }
          label="Show Premium Template"
        />
        </Box>
      </section>

           {
                showPremiumTemplate ? <PremiumPreview formData={formData}/> : (
                <BusinessPreview 
                    formData={formData} 
                />)
           }
